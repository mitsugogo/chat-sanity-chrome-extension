...
- 判定順序は「無効化・配信者／モデレーター／自分／ホワイトリスト除外 → 許可語句 → ブロック語句 → 非表示ユーザー → カテゴリルール → スパム → セッション加重 → 必要時のみLM Studio」。
- 初期閾値は`< 0.50: 表示`、`0.50–0.74: 薄く表示`、`0.75–0.89: ぼかし`、`>= 0.90: 非表示`。
- 通常のAI対象はルールスコア`0.35–0.80`の曖昧域とする。加えて、LM Studio有効時は`unmatched`かつ0点の一部だけを、レート制限・弱いシグナル・同文キャッシュ付きのZero-score Auditで監査してよい。
...
- `chrome.storage.sync`に保存するのは`schemaVersion: 1`の設定だけ。コメント本文、診断履歴、セッション集計を保存しない。非表示ユーザーとホワイトリストはチャンネルIDと表示名だけを保存してよい。
